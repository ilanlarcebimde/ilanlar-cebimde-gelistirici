...
import type { CoverLetterResult } from "../lib/coverLetterSchema";
import { COVER_LETTER_RESULT_UI } from "@/components/apply/coverLetterWizardContent";

const DEFAULT_TR = COVER_LETTER_RESULT_UI.noticeTr;
const DEFAULT_EN = COVER_LETTER_RESULT_UI.noticeEn;

type TabId = "tr" | "en";

export interface StepResultTabsProps {
  data: CoverLetterResult;
  jobEmail?: string | null;
  onClose?: () => void;
}

export function StepResultTabs({ data, jobEmail, onClose }: StepResultTabsProps) {
  const [activeTab, setActiveTab] = useState<TabId>("tr");
  const [copied, setCopied] = useState<TabId | null>(null);

  const trNotice = data.ui_notes?.tr_notice ?? DEFAULT_TR;
  const enNotice = data.ui_notes?.en_notice ?? DEFAULT_EN;
        ...
        <div className="sticky bottom-0 z-10 mt-4 flex flex-col gap-2 border-t border-slate-100 bg-white pt-4 md:flex-row md:items-center md:gap-3">
          {activeTab === "tr" && (
            <button
              type="button"
              onClick={copyTr}
              className="h-12 w-full rounded-2xl bg-slate-900 font-semibold text-white hover:bg-slate-800 md:min-w-[180px] md:px-6"
            >
          ...
          {activeTab === "en" && (
            <>
              <button
                type="button"
                onClick={copyEn}
                className="h-12 w-full rounded-2xl bg-slate-900 font-semibold text-white hover:bg-slate-800 md:min-w-[160px] md:px-6"
              >
              ...
              {jobEmail && (
                <a
                  ...
                  className="flex h-12 items-center justify-center rounded-2xl border border-slate-200 bg-white font-medium text-slate-700 hover:bg-slate-50 md:min-w-[200px] md:px-6"
                >
